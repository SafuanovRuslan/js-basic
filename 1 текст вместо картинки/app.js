...
function handleClick(clickedButtonEvent) {
    ...
    const cardNode = clickedButtonEvent.target.closest('.product');
    ...
    const card = {
        ...
        productName: cardNode.querySelector('div'), // здесь .productName, который внутри .product
...
function showMoreText(card) {
    ...
    const descriptionText = 'Какое-то описание ' + card.productName.textContent;
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    const description = document.createElement('div');
    description.classList.add('desc');
    description.textContent = descriptionText;
    card.productName.insertAdjacentElement('afterend', description);
